...
#define VersionNumber "Subsystem Version 1.5.1 (Release)" //1.5.1 only adds optional debug prints as opposed to 1.5
...
void setup(void) {
  ...
  while(!Serial);
  tft.begin();
  if(!SD.begin(SD_CS, SD_SCK_MHZ(25))) { //This is where the infamous NOSD error comes from, this only occurs if there is a problem initializing the sd card
  ...
  if(tempScan == 0){ // if section 1 button is pressed
    ...
    if(opened){ // if file exists
      if(rotary == newPos){ // if rotary number matches solution
        ...
        sendUpdate(true); // tell serial adapter that they passed this puzzle (outdated) (original way we were going to integrate, and will revise here later on for thorium web server)
...
void section2(){
  ...
  if(tempScan == 0){ // if button was pressed
    ...
    for(x = 0; x < 10; x++){ //go through each jack as a test point
      ...
      delayMicroseconds(1); // give it time to acctuate
      ...
      delayMicroseconds(1); // give it a microsecond to comply
      pinMode(JackPins[x], INPUT_PULLUP); // set it to pullup
      delay(10); // give it 1/100 of a second to stabilize
      if(i == 5){
...
void readFile(){
 ...
 void sendData(){
  ...
  setup(); // then immediatly reset the panel to its original state
 }
 byte findIndex(byte list[], byte valueWanted){ // function to find a specific value in an array
  for(byte j = 0; j < 7; j++){
 ...
 void sendUpdate(bool Didpass){ // send updates to thorium (right now still is the outdated serial - usbgamepad way) (Idealy this will just print the same number scheme to a webpage)
  for(i = 0; i < 8; i++){ // print puzzle number
    if(switches[7-i]){
      Serial.print(0);
    }
    else{
  ...
  if(section == 0){ // what section we are on
    Serial.print("100");
  }
  else if(section == 1){
    Serial.print("010");
  ...
  if(Didpass){ // and finally if they got it right or wrong
    Serial.println(1);
  }
  else{
    Serial.println(0);
