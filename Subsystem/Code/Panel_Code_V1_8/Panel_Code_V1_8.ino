...
DESIGNED AND PROGRAMMED BY NATHAN FORD 2020 - UPDATED 2024
*/
...
#define VersionNumber "Subsystem Version 1.8 (Release)" //1.8 is for PCB models
...
void setup(void) {
  ...
  #if DEBUGPRINTS
    Serial.print("Screen print code: ");
    Serial.println(stat);
  #endif
  ...
  if(tempScan == 0){ // if section 1 button is pressed
    ...
    if(opened){ // if file exists
      if(rotary == newPos){ // if rotary number matches solution
        ...
        storeUpdate(true); // tell serial adapter that they passed this puzzle (outdated) (original way we were going to integrate, and will revise here later on for thorium web server)
...
void reset(){
 ...
 void storeUpdate(bool Didpass){ // prepare updates to thorium
  for(int i = 0; i < 12; i++){
    ethernetCode[i] = 0;
  }
